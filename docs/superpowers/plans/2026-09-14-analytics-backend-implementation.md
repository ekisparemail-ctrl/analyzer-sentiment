...
- Running locally for development: `python -m venv .venv && .venv\Scripts\pip install -r requirements.txt && .venv\Scripts\python src\main.py` (Windows), with a real `.env` (copy from `.env.example`) in the working directory `Settings` is run from -- `Settings` (`src/config.py`) auto-loads `.env` via `SettingsConfigDict(env_file=".env")` (fixed post-implementation, see `docs/issue-findings.md`; it originally read only `os.environ` and did not load `.env` at all, which broke running the service straight after copying `.env.example`). `ffmpeg`/`ffprobe` must be installed and on PATH (not a pip package).
